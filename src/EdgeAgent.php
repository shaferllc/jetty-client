...
final class EdgeAgent
{
    ...
    public static function websocketPingIntervalSeconds(): float
    {
        ...
        $v = (float) $raw;
        if (
            $v < self::MIN_WS_PING_INTERVAL_SECONDS ||
            $v > self::MAX_WS_PING_INTERVAL_SECONDS
        ) {
            return self::DEFAULT_WS_PING_INTERVAL_SECONDS;
        }
    ...
    public static function run(
        ...
        $rewriteOpts =
            $rewriteOptions ?? TunnelRewriteOptions::fromEnvironment();
        $heartbeatAgentDebug =
            $agentDebug !== null &&
            EdgeAgentDebug::heartbeatEventsFromEnvironment();
        ...
        async(function () use (
            ...
            try {
                ...
                $stderr(
                    'edge: signal handlers unavailable: '.$e->getMessage(),
                );
            }
                ...
                while ($state->running) {
                    ...
                    try {
                        ...
                        if ($heartbeatAgentDebug) {
                            self::agentEmit($agentDebug, 'heartbeat_ok', [
                                'n' => $n,
            ...
            try {
                ...
                while (! $state->userRequestedStop) {
                    ...
                    if ($session > 1 && $edgeReconnect) {
                        ...
                        $stderr(
                            sprintf(
                                'edge: reconnecting in %.1fs (attempt %d)…',
                                $wait,
                                $session,
                            ),
                        );
                        ...
                        $stderr(
                            'edge: WebSocket closed; HTTP forwarding paused (JETTY_SHARE_NO_EDGE_RECONNECT=1). Heartbeats continue until you exit.',
                        );
                    ...
                    try {
                        ...
                        self::agentEmit($agentDebug, 'ws_connect_failed', [
                            'session' => $session,
                            'error_class' => $e::class,
                            'error_detail' => self::formatEdgeWsConnectErrorDetail(
                                $e,
                            ),
                        ]);
                        $stderr(
                            'edge WebSocket connect failed: '.
                                self::formatEdgeWsConnectErrorDetail($e),
                        );
                    ...
                    $v(
                        'WebSocket connected; sending register (tunnel_id='.
                            $tunnelId.
                            ', agent_token len='.
                            strlen($agentToken).
                            ')',
                    ...
                    if ($type === self::TYPE_ERROR) {
                        ...
                        $conn->close();
                        $stale = self::registrationErrorLooksLikeStaleCredentials(
                            $errMsg,
                        );
                        if (
                            $stale &&
                            $refreshAgentToken !== null &&
                            $agentTokenRefreshCount <
                                self::MAX_AGENT_TOKEN_REFRESH_PER_RUN
                        ) {
                            try {
                                $stderr(
                                    'edge: refreshing agent_token via Bridge attach…',
                                );
                                ...
                                $stderr(
                                    'edge: could not refresh agent_token: '.
                                        $e->getMessage(),
                                );
                            }
                        }
                    ...
                    if ($type !== self::TYPE_REGISTERED) {
                        self::agentEmit(
                            $agentDebug,
                            'edge_unexpected_first_frame',
                            [
                                'session' => $session,
                                'frame_type' => $type,
                                'preview' => self::debugStringPreview(
                                    $rawFirst,
                                    400,
                                ),
                            ],
                        ...
                        if ($verbose) {
                            $stderr(
                                '[jetty:edge] full first message: '.$rawFirst,
                            );
                        }
                        $conn->close();
                        if ($state->userRequestedStop) {
                    ...
                    if ($session === 1) {
                        $stderr(
                            'Edge agent connected; forwarding HTTP to local upstream.',
                        );
                    } else {
                        $stderr(
                            'Edge agent reconnected; forwarding HTTP to local upstream.',
                        );
                    }
                    $v(
                        'registration acknowledged; starting ws ping + receive loop',
                    );
                    ...
                    try {
                        ...
                        while ($state->running) {
                            try {
                                $msg = $conn->receive(
                                    $cancelRead->getCancellation(),
                                );
                            } catch (CancelledException $e) {
                                self::agentEmit(
                                    $agentDebug,
                                    'ws_receive_cancelled',
                                    [
                            ...
                            if ($msg === null) {
                                ...
                                $v(
                                    'receive() returned null — connection closed',
                                );
                            ...
                            if ($ft !== self::TYPE_HTTP_REQUEST) {
                                ...
                                $v(
                                    'frame #'.
                                        ...
                                        ' type='.
                                        $ft.
                                        ' (ignored, not http_request)',
                                );

                                continue;
                            }
                            TunnelResponseRewriter::emitDebugNdjson(
                                'edge.ws_http_request_frame',
                                [
                                    'msg_num' => $msgNum,
                                    'raw_bytes' => strlen($raw),
                                ],
                            ...
                            try {
                                ...
                                TunnelResponseRewriter::emitDebugNdjson(
                                    'edge.http_request_json_invalid',
                                    [
                                        'msg_num' => $msgNum,
                                        'error' => self::debugStringPreview(
                                            $e->getMessage(),
                                            300,
                                        ),
                                        'raw_bytes' => strlen($raw),
                            ...
                            try {
                                $handled = self::handleHttpRequest(
                                    $localHost,
                                    $localPort,
                                    $req,
                                    $rewriteOpts,
                                ...
                                TunnelResponseRewriter::emitDebugNdjson(
                                    'edge.http_request_handler_error',
                                    [
                                        'msg_num' => $msgNum,
                                        'request_id' => (string) ($req['request_id'] ?? ''),
                                        'error_class' => $e::class,
                                        'error_message' => self::debugStringPreview(
                                            $e->getMessage(),
                                            500,
                                        ),
                                    ],
                            ...
                            $v(
                                'http_response sent for request_id='.
                                    (string) ($req['request_id'] ?? ''),
                            );
                            // Always-on traffic log line
                            $stderr(
                                self::formatTrafficLine(
                                    $method,
                                    $path,
                                    $respStatus,
                                    strlen($outJson),
                    ...
                    if ($state->userRequestedStop) {
                        self::agentEmit($agentDebug, 'session_user_stop', [
                            'session' => $session,
                        ]);
                        $box->result = EdgeAgentResult::Finished;
                    ...
                    if (! $edgeReconnect) {
                        self::agentEmit(
                            $agentDebug,
                            'session_disconnected_no_reconnect',
                            ['session' => $session],
                        );
                        $box->result = EdgeAgentResult::Disconnected;
                    ...
                    self::agentEmit(
                        $agentDebug,
                        'ws_session_closed_will_reconnect',
                        [
                            'session' => $session,
                    ...
                    $stderr(
                        'edge: WebSocket dropped; will retry (Ctrl+C to stop).',
                    );
                }
    ...
    private static function registrationErrorLooksLikeStaleCredentials(
        string $message,
    ): bool {
    ...
    private static function handleHttpRequest(
        ...
        // Phase D: Per-request routing (Option 1 - edge decides routing).
        // If the edge provides explicit UpstreamHost/UpstreamPort for this request,
        // override the tunnel's default local_host/local_port for path/header routing.
        $upstreamHostOverride = isset($req['upstream_host'])
            ? trim((string) $req['upstream_host'])
            : '';
        $upstreamPortOverride = isset($req['upstream_port'])
            ? (int) $req['upstream_port']
            : 0;
        if ($upstreamHostOverride !== '' && $upstreamPortOverride > 0) {
            $localHost = $upstreamHostOverride;
            $localPort = $upstreamPortOverride;
        }
        ...
        self::agentEmit($agentDebug, 'http_request_in', [
            ...
            'edge_host' => $edgeHost ?? '',
            'edge_headers_redacted' => EdgeAgentDebug::redactSensitiveRequestHeaders(
                $headers,
            ),
        ]);
        ...
        if (! $hostPolicy->allows($localHost)) {
            ...
            return [
                'response' => self::errorResponse(
                    $requestId,
                    502,
                    'upstream host not allowed by JETTY_SHARE_UPSTREAM_ALLOW_HOSTS',
                ),
                'sample' => self::samplePayload(
                    $method,
                    $path,
                    $query,
                    $headers,
        ...
        self::agentEmit($agentDebug, 'http_upstream_begin', [
            'request_id' => $requestId,
            'target' => $target,
            'upstream_host_header' => TunnelUpstreamRequestHeaders::upstreamHostHeaderValue(
                $localHost,
                $localPort,
            ),
        ...
        if ($response === false) {
            ...
            TunnelResponseRewriter::emitDebugNdjson(
                ...
                [
                    ...
                    'invocation_cwd' => is_string($invCwdLog) && $invCwdLog !== ''
                            ? $invCwdLog
                            : null,
                    'cli_upstream_hostname' => is_string($cliLog) && trim($cliLog) !== ''
                            ? trim($cliLog)
                            : null,
                ],
            );
        ...
        $parsedHeaders = self::removeHeaderCi(
            $parsedHeaders,
            'Content-Encoding',
        );
        ...
        $rewriteRequestHeaders = TunnelResponseRewriter::requestHeadersWithRewriteTunnelHostFallback(
            $headers,
            $publicTunnelHostForRewrite,
        );
        TunnelResponseRewriter::debugRewriteRequestContext(
            $requestId,
            $method,
            $path,
            $localHost,
        ...
        $outHeaders = TunnelResponseRewriter::rewriteRedirectHeaders(
            $parsedHeaders,
            $rewriteRequestHeaders,
            $lookup,
            $localHost,
        ...
        $cliLog = getenv('JETTY_SHARE_CLI_UPSTREAM_HOSTNAME');
        $locBefore =
            self::headerValueCi($parsedHeaders, 'Location') ??
            self::headerValueCi($parsedHeaders, 'X-Inertia-Location');
        $locAfter =
            self::headerValueCi($outHeaders, 'Location') ??
        ...
        TunnelResponseRewriter::emitDebugNdjson('edge.http_upstream_lookup', [
            ...
            'cli_upstream_hostname' => is_string($cliLog) && trim($cliLog) !== ''
                    ? trim($cliLog)
                    : null,
            'location_before' => $locBefore !== null
                    ? self::debugStringPreview($locBefore, 512)
                    : null,
            'location_after' => $locAfter !== null
                    ? self::debugStringPreview($locAfter, 512)
                    : null,
        ]);
        $bodyBeforeTunnelRewrite = $respBody;
        $respBody = TunnelResponseRewriter::maybeRewriteBody(
            $respBody,
            $outHeaders,
            $rewriteRequestHeaders,
            $localHost,
        ...
        if (str_contains($ct, 'text/html')) {
            $viteHits = TunnelResponseRewriter::detectViteDevServerUrls(
                $bodyBeforeTunnelRewrite,
                $lookup,
                $localPort,
            );
            $effectiveTunnelHost =
                self::headerValueCi($rewriteRequestHeaders, 'Host') ??
                $publicTunnelHostForRewrite;
            TunnelResponseRewriter::emitViteDevServerWarning(
                $viteHits,
                $requestId,
                $localHost,
                $localPort,
            ...
            if ($viteHits !== []) {
                $respBody = TunnelResponseRewriter::injectViteDevServerBanner(
                    $respBody,
                    $viteHits,
                    $localHost,
                    $localPort,
        ...
        self::agentEmit($agentDebug, 'http_upstream_response', [
            ...
            'response_headers_redacted' => EdgeAgentDebug::redactSensitiveResponseHeaders(
                $outHeaders,
            ),
        ]);
        self::agentEmit($agentDebug, 'http_tunnel_rewrite', [
            ...
            'redirect_header_changes' => self::redirectHeaderChanges(
                $beforeRedirectRewrite,
                $outHeaders,
            ),
     ...
     */
    private static function samplePayload(
        string $method,
        string $path,
        string $query,
        array $headers,
     ...
     */
    private static function agentEmit(
        ?callable $cb,
        string $event,
        array $data = [],
    ): void {
     ...
     */
    private static function formatTrafficLine(
        string $method,
        string $path,
        int $status,
        int $bytes,
        ...
        $pathDisplay =
            strlen($path) > 60 ? substr($path, 0, 57).'...' : $path;
     ...
     */
    private static function redirectHeaderChanges(
        array $before,
        array $after,
    ): array {
    ...
    private static function maybeReportWsFailureTelemetry(
        ApiClient $apiClient,
        \Throwable $e,
    ): void {
        ...
        try {
            ...
            $apiClient->postEdgeWsFailureTelemetry(
                $status,
     ...
     */
    private static function formatEdgeWsConnectErrorDetail(
        \Throwable $e,
    ): string {
        $msg = $e->getMessage();
        if (! $e instanceof WebsocketConnectException) {
        ...
        $headerParts = [];
        foreach (
            ['Server', 'Via', 'CF-Ray', 'X-Cache', 'X-Served-By', 'Date'] as $name
        ) {
            $v = $r->getHeader($name);
            if ($v !== null && $v !== '') {
                $headerParts[] = $name.': '.$v;
        ...
        if (in_array($status, [502, 503, 504], true)) {
            $lines[] =
                '  Hint: '.
                $status.
                ' on upgrade usually means the TLS proxy could not reach jetty-edge (check edge nginx/journalctl); Laravel verify runs only after 101 Switching Protocols.';
        }
